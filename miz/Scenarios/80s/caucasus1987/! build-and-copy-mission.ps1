...
try {
    ...
    # Copy to DCS Saved Games only after a successful mission build (CFG + *_fowl_export.json next to writedir root; .miz under Missions\)
    if ($buildSuccess) {
        ...
        $cfgSrc = Join-Path -Path $PSScriptRoot -ChildPath $target_cfg_name
        $cfgDst = Join-Path -Path $DCS_user_path -ChildPath $target_cfg_name
        $cfg_copy_msg = "Copying $target_cfg_name to $cfgDst"
        Write-Host $cfg_copy_msg -ForegroundColor Cyan
        $cfg_copy_msg | Out-File -FilePath $log_file -Append
        Copy-Item -LiteralPath $cfgSrc -Destination $cfgDst -Force -ErrorAction Stop

        $exportSrc = Join-Path -Path $PSScriptRoot -ChildPath $fowlExport
        $exportDst = Join-Path -Path $DCS_user_path -ChildPath $fowlExport
        $export_copy_msg = "Copying $fowlExport to $exportDst (same folder as CFG)"
        Write-Host $export_copy_msg -ForegroundColor Cyan
        $export_copy_msg | Out-File -FilePath $log_file -Append
        Copy-Item -LiteralPath $exportSrc -Destination $exportDst -Force -ErrorAction Stop

        $mizSrc = Join-Path -Path $PSScriptRoot -ChildPath "$mission_name.miz"
        $mizDst = Join-Path -Path (Join-Path -Path $DCS_user_path -ChildPath "Missions") -ChildPath "$mission_name.miz"
        $miz_copy_msg = "Copying $mission_name.miz to $mizDst"
