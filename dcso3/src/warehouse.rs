...
impl<'lua> WSType<'lua> {
    pub fn category(&self) -> Result<WSCategory> {
        match self.t.raw_get(1)? {
            ...
            1 => match self.t.raw_get(2)? {
                ...
                // Droptanks: aircraft branch (1,3), not wsType level1==4 (Weapons).
    ...
    /// DCS `wsType` quad `{1,2,3,4}` (Lua 1-based table indices).
    pub fn quad(&self) -> Result<[i32; 4]> {
        fn idx(t: &LuaTable<'_>, i: i32) -> Result<i32> {
            let v: Value<'_> = t.raw_get(i)?;
            match v {
                Value::Integer(n) => Ok(n as i32),
                Value::Number(n) => Ok(n as i32),
                _ => Err(anyhow!("WSType[{}]: expected number, got {:?}", i, v)),
            }
        }
        Ok([
            idx(&self.t, 1)?,
