...
pub struct Ephemeral {
    ...
    /// New round only: `-action` disabled until this wall-clock time.
    pub(super) actions_unlock_after: Option<DateTime<Utc>>,
...
impl Ephemeral {
    ...
    pub fn slot_for_ucid(&self, ucid: &Ucid) -> Option<SlotId> {
        self.players_by_slot
            .iter()
            .find_map(|(slot, u)| if *u == *ucid { Some(*slot) } else { None })
    }
