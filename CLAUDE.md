...
After a successful **FowlTools** `miz` build, **`{mission_stem}_fowl_export.json`** is written next to the output `.miz` (weapon `wsType` allowlists from built **BINVENTORY** / **RINVENTORY** only, not **BDEFAULT** / **RDEFAULT**). The assembled mission **BINVENTORY** / **RINVENTORY** rows are the **bftools-built** production inventory (validate/prune/`+` merge on the warehouse template copy), re-applied after all warehouse passes; objective warehouses still use **BDEFAULT**/**RDEFAULT** + **BINVENTORY** during fill. Copy export JSON to **`Saved Games\DCS`** next to **`{sortie}_CFG`**; **bflib** **requires** it (`writedir` + `<sortie>_fowl_export.json`) — mission start fails if missing.
